...
開發者在串接金流（藍新 NewebPay、綠界 ECPay、Stripe）時，需要反覆在測試頁面手動輸入卡號、到期日、CVC 來驗證「成功」與「各種失敗情境」。每家金流的測試卡號分散在各自文件、且失敗情境卡號繁多，手動查找與輸入既慢又容易打錯。本外掛在測試頁面提供一鍵填入，省去查文件與手打的成本。
...
- 內建三家金流（綠界、藍新、Stripe）官方測試卡資料庫，涵蓋成功卡與失敗情境卡（餘額不足、卡片被拒、CVC 錯誤、過期、3DS 驗證等，依各家文件實際提供者為準）。
