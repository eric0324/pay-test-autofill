...
### D5：打包用 Bun.build，輸出雙瀏覽器
`build.mjs`（以 `bun build.mjs` 執行）：`Bun.build` 將 `src/content/index.js` bundle 成 IIFE 單檔 → 寫入 `dist/chrome` 與 `dist/firefox`，各自搭配 `manifest.chrome.json` / `manifest.firefox.json`。替代方案：不打包、manifest 列多檔共享全域 —— 較鬆散易出錯。打包與測試（`bun test`）皆用 Bun 內建工具，零額外建構相依。
