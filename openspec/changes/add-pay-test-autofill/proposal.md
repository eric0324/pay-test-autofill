...
- 提供 `npm run build`，由單一份 `src/` 產出 `dist/chrome/` 與 `dist/firefox/` 兩份可載入的外掛。
...
- `cross-browser-build`: 由單一份原始碼經 esbuild 打包，產出 Chrome 與 Firefox 兩種 Manifest V3 外掛的建構流程。
...
- 全新專案，無既有程式碼。
- 新增開發相依：`esbuild`（打包）。執行期外掛本身零執行相依。
